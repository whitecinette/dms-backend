    ...
    exports.updateWeeklyBeatMappingStatusWithProximity = async (req, res) => {
        try {
            ...
            if (!status || !employeeLat || !employeeLong) {
                return res.status(400).json({ error: "status, employeeLat, and employeeLong are required in the request body." });
            }
            ...
            if (!dealer) {
                return res.status(404).json({ error: "Dealer not found in the schedule." });
            }
            ...
            const distance = getDistance(employeeLat, employeeLong, dealerLat, dealerLong); // Using the given function
