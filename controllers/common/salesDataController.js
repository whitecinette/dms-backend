...
exports.getSalesReportForUser = async (req, res) => {
  try {
    ...
    const selectedProductCategories = subordinate_codes ? subordinate_codes.filter(code => productCategories.includes(code)) : [];
...
exports.getDashboardSalesMetricsForUser = async (req, res) => {
  try {
    ...
    const productCategories = ['smart_phone', 'tab', 'wearable'];
    const hasProductCategories = subordinate_codes && subordinate_codes.some(code => productCategories.includes(code));
    const selectedProductCategories = subordinate_codes ? subordinate_codes.filter(code => productCategories.includes(code)) : [];
    ...
    const getTotal = async (salesType, dateRange) => {
      if (hasProductCategories) {
        ...
        const total = filteredSalesData.reduce((sum, sale) => {
          const value = filter_type === "value" ? parseFloat(sale.total_amount) : parseFloat(sale.quantity);
    ...
    const mtdSellIn = await getSellinTotal(["Sell In", "Sell Thru2"], { start: startDate, end: endDate });
    const lmtdSellIn = await getSellinTotal(["Sell In", "Sell Thru2"], { start: lmtdStartDate, end: lmtdEndDate });
...
exports.getSalesReportProductWise = async (req, res) => {
  try {
    ...
    const target = await Target.findOne({ entity: code }); // ← allow null
    // If not found, treat as empty target object
